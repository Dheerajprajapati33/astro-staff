...
export default function RefundPolicyScreen() {
  // Type ke basis par fetch
  const { data, isLoading, isError, refetch } = useGetAppContentQuery("refund_policy");

  if (isLoading)
    return <ActivityIndicator size="large" color={Colors.primary} />;

  if (isError)
    return <Text>Error loading content. Please try again.</Text>;
